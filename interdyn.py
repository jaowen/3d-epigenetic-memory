...
def update_marks_SIS(pts, marks, r, s, ET, p3D, tmax):
    ...
    for _ in range(tmax):
        ...
        msim = EoN.fast_SIS(G, s*pb, 1.0, initial_infecteds=marked_sites, tmax=1.0, transmission_weight='weight', return_full_data=True)
...
def main():
    ...
    collrate = 2.0 #not actually used, since no dynamics here
    alpha = 0 / 10.0
    p3D = 100 / 100.0
    rtime = 1000
   ...
   # marks0 = 2000*[-1.0] + 1000*[1.0] + 4000*[-1.0] + 1000*[1.0] + 2000*[-1.0]
    ...
    for alpha10 in alpha10range:
        ...
        for s100 in s100range:
            s = s100 / 100.0
            print((a, s))
            csvfile = open('interdyn_a'+str(alpha10)+'_s'+str(s100)+'_e'+str(ET)+'_dyn'+str(dynspeed)+'_rt'+str(rtime)+'.csv', 'w', newline='')
            ...
            for i in range(gens):
                ...
                if count_marks(marks) == 0:
                    ...
                    for j in range(200):
                        #print(j)
                        pts = sim.get_data()
                        #polymerutils.save(pts, 'dyntestmanygens_a'+str(alpha10)+'_j'+str(j)+'.dat')
